...
>
> 确认方法：`运行计划` 区块右下角「下一次运行」列表下方有一行小字
> `In this job's individual timezone (Asia/Shanghai)`，同时核对那几个时刻是否等于你期望的北京时间。
> 不符就改；如果时区改不了，就按 UTC 填 crontab（见 4.3 末尾的换算表）。
...
> ⚠️ **cron 表达式的顺序是「分 时 日 月 周」，不是「时 分」** —— 这是最经典的填错点：
> 想把「8 点整」写成 `8 0 * * *`，实际得到的是 **00:08**（分钟=8、小时=0），而不是 08:00。
> 正确写法是 **`0 8 * * *`**。
>
> **填完必须核对「Next execution」**：猫猫出发那一行要显示 `8:00:00 AM`；
> 若显示 `12:08:00 AM`（= 00:08），就是顺序写反了。
> 三个 job 的自检值：签到 `12:05:00 AM`、猫猫出发 `8:00:00 AM`、猫猫领取 `12:30:00 PM`。

若改用自定义 cron 表达式：

```
# Timezone = Asia/Shanghai 时直接填这些（注意：第 1 位是「分钟」）
5 0 * * *      # 签到 00:05
0 8 * * *      # 猫猫出发 08:00   ← 不是 8 0
30 12 * * *    # 猫猫领取 12:30
